...
@click.option('--model_fea_length',
              default=DEFAULT_CONFIG.fil.model_fea_length,
              type=click.IntRange(min=1),
              help="Features length to use for the model")
@prepare_command(Config.get().fil)
...
def preprocess_fil(ctx: click.Context, **kwargs):
    ...
    stage = UserProfPreprocessingStage(Config.get(), **kwargs)
...
def serialize(ctx: click.Context, **kwargs):
    ...
    stage = UserProfSerializeStage(Config.get(), **kwargs)
